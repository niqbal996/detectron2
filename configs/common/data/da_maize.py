...
import torchvision
source_dataloader = OmegaConf.create()
target_dataloader = OmegaConf.create()

source_dataloader.train = L(build_detection_train_loader)(
...
source_dataloader.test = L(build_detection_test_loader)(
...
source_dataloader.evaluator = L(COCOEvaluator)(
    dataset_name="${..test.dataset.names}",
    max_dets_per_image=500,
)

target_dataloader.train = L(build_detection_train_loader)(
    dataset=L(get_detection_dataset_dicts)(names="maize_real_v2_val"),
    mapper=L(DatasetMapper)(
        is_train=True,
        augmentations=[
            L(T.ResizeShortestEdge)(
...
target_dataloader.test = L(build_detection_test_loader)(
    dataset=L(get_detection_dataset_dicts)(names="maize_real_v2_val", filter_empty=True),
    mapper=L(DatasetMapper)(
        is_train=True,
        augmentations=[
...
target_dataloader.evaluator = L(COCOEvaluator)(
    dataset_name="${..test.dataset.names}",
    max_dets_per_image=500,
)
