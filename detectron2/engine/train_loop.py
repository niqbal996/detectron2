...
class DATrainer(TrainerBase):
    """
    A Domain Adapatation trainer for Scale aware domain adaptation faster RCNN:
    single-cost single-optimizer two-data-sources iterative optimization,
    optionally using data-parallelism.
    It assumes that every step, you:
    ...
    def __init__(
        ...
        # to access the data loader iterator, call `self._data_loader_iter`
        self._source_data_loader_iter_obj = None
    ...
    def run_step(self):
        ...
        If you want to do something with the data, you can wrap the dataloader.
        """
        source_data, target_data = next(self._source_data_loader_iter), next(self._target_data_loader_iter)
    ...
    @property
    def _source_data_loader_iter(self):
        # only create the data loader iterator when it is used
        if self._source_data_loader_iter_obj is None:
            self._source_data_loader_iter_obj = iter(self.source_data_loader)
        return self._source_data_loader_iter_obj
    ...
    def reset_data_loader(self, data_loader_builder):
        ...
        by calling `data_loader_builder` (without argument).
        """
        del self.source_data_loader
        del self.target_data_loader
        source_data_loader = data_loader_builder()
        target_data_loader = data_loader_builder()
        self.source_data_loader = source_data_loader
        self.target_data_loader = target_data_loader
        self._source_data_loader_iter_obj = None
        self._target_data_loader_iter_obj = None
